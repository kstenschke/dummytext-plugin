...
public class generateMedicalAction extends AnAction {
		...
		CommandProcessor.getInstance().executeCommand(currentProject, new Runnable() {
			public void run() {
				ApplicationManager.getApplication().runWriteAction(new Runnable() {
					public void run() {
						new ActionPerformer("hospital").write(event);
					}
				});
			}
		}, "Insert Medical Drama", UndoConfirmationPolicy.DO_NOT_REQUEST_CONFIRMATION);
	}
